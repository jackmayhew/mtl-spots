  ...
  return (
    <div className="pagination p1">
      <ul>
        ...
        {/* first */}
        {currentPage > 2 && (
          <Link scroll={false} href={`/saved?page=1`}>
            <a
              onClick={() =>
        ...
        {/* prev */}
        {currentPage >= 2 && (
          <Link scroll={false} href={`/saved?page=${previousPage}`}>
            <a
              onClick={() =>
                window.scroll({ top: 0, left: 0, behavior: "smooth" })
              }
        ...
        {/* current */}
        {count >= 13 && (
          <Link scroll={false} href={`/saved?page=${currentPage}`}>
            <a
              className="is-active"
              onClick={() =>
                window.scroll({ top: 0, left: 0, behavior: "smooth" })
        ...
        {/* next */}
        {hasNextPage && (
          <Link scroll={false} href={`/saved?page=${nextPage}`}>
            <a
              onClick={() =>
                window.scroll({ top: 0, left: 0, behavior: "smooth" })
              }
        ...
        {nextPage !== lastPage && currentPage !== lastPage && count > 13 && (
          <Link scroll={false} href={`/saved?page=${lastPage}`}>
            <a
              onClick={() =>
                window.scroll({ top: 0, left: 0, behavior: "smooth" })
              }
